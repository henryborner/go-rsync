// NEON checksum v11: 128B unrolled (4x32B per SUB/CBNZ). VUADDLV s1 + WORD VUMULL s2.
// All 8 VUMULL per 64B fire before mid-iteration VLD1 — no data race, max latency hide.
//
...
loop:
	// ====== Part 1: first 64B (preloaded) ======
	MOVD    R8, R10

	// Fire ALL 8 VUMULLs before VLD1 overwrites data
	...
	// V2,V3,V20,V21 now free. Load second 64B.
	VLD1.P  32(R0), [V2.B16, V3.B16]
	VLD1.P  32(R0), [V20.B16, V21.B16]

	// Reduce part 1
	VADDP   V6.H8, V5.H8, V5.H8
	WORD    $0x4E6028A5         // VSADDLP V5.4S, V5.H8
	VADDP   V8.H8, V7.H8, V7.H8
	WORD    $0x4E6028E7         // VSADDLP V7.4S, V7.H8
	VADD    V5.S4, V7.S4, V5.S4

	VADDP   V25.H8, V24.H8, V24.H8
	WORD    $0x4E602B18         // VSADDLP V24.4S, V24.H8
	VADDP   V27.H8, V26.H8, V26.H8
	WORD    $0x4E602B5A         // VSADDLP V26.4S, V26.H8
	VADD    V24.S4, V26.S4, V24.S4

	VADD    V5.S4, V24.S4, V5.S4
	VADD    V12.S4, V5.S4, V12.S4
