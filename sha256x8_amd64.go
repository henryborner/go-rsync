...
// sha256x8corePureGo is a pure Go 8-way SHA-256 core for debugging.
func sha256x8corePureGo(x *[16][8]uint32, state *[8][8]uint32, wbuf *[16][8]uint32, saved *[8][8]uint32) {
	// Save initial state
	*saved = *state

	// Use a flat 64-word schedule (not circular!) to avoid confusion.
	var w [64][8]uint32

	// Rounds 0-15: load from x (already BE from Go glue code, no BSWAP needed)
	for i := 0; i < 16; i++ {
		for lane := 0; lane < 8; lane++ {
			w[i][lane] = x[i][lane]
		}
	}
	// Rounds 16-63: compute message schedule
	for i := 16; i < 64; i++ {
		for lane := 0; lane < 8; lane++ {
