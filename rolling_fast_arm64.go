...
// Checksum1Components returns the raw (s1, s2) components. Unlike amd64
// (16-bit-truncated since v7), arm64 returns full 32-bit values.
...
func checksum1(data []byte) (uint32, uint32) {
	...
	// Same structure as the amd64 pure-Go fallback (verified parity).
	for i+128 <= n {
		// Batch 0 (bytes 0..31)
		var g0 [8]uint32
		var sw0 uint32
	...
	// Scalar remainder tail (< 32 bytes).
	for ; i < n; i++ {
		s1 += uint32(data[i]) + CHAR_OFFSET
		s2 += s1
	}
